...
module.exports = function(config) {
    ...
    return new Promise( async function (resolve, reject)  {
      ...
      for(let i=0;i<deviceList.length;i++) {
        if( deviceList[i].deviceModel == "HS110(EU)") {
            ...
            meter.externalAccount = meter.deviceId+"_"+meter.hwId+"_"+meter.deviceMac;
            meter.meterId = meter.hwId+"_"+meter.deviceMac;
  ...
  this.switchOn = async function(query) {
    let parent = this;
    return new Promise( async function (resolve, reject)  {
      const tplink = await login(config.TPLINK_ACCOUNT, config.TPLINK_PASSWORD, "TermID");
      let deviceList = await tplink.getDeviceList();
      ...
      for(let i=0;i<deviceList.length;i++) {
        if( deviceList[i].deviceModel == "HS110(EU)") {
            let meter = deviceList[i];
            let found=false;
            if(meter.alias == query) found=true;
            if(meter.deviceId == query) found=true;
            if(meter.deviceMac == query) found=true;
            if(meter.hwId == query) found=true;
            if(meter.oemId == query) found=true;
            if(found) {
              await tplink.getHS110(meter.alias).powerOn();
              res = {switched:true};
            }
        }
      }
